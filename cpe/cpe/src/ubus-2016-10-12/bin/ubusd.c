...
{
	printf("%s|ubus|ubusd|ubus_msg_free\n",current_time());
	switch (ub->refcount) {
	case 1:
	case ~0:
		if (ub->fd >= 0)
...
{
	printf("%s|ubus|ubusd|ubus_msg_writev\n", current_time());
...
{
	printf("%s|ubus|ubusd|ubus_msg_send\n",current_time());
	int written;
...
{
	printf("%s|ubus|ubusd|handle_client_disconnect\n",current_time());
...
{
	printf("%s|ubus|ubusd|client_cb|events: %d\n",current_time(), events);
...
retry:
	if (!sock->eof && cl->pending_msg_offset < sizeof(cl->hdrbuf)) {
		...
		if (bytes < 0) {
			printf("%s|ubus|ubusd|client_cb| recv msg error, bytes: %d\n",current_time(), bytes);
			goto out;
		}
		...
		if (cl->pending_msg_offset < sizeof(cl->hdrbuf)) {
			printf("%s|ubus|ubusd|client_cb| recv msg error, cl->pending_msg_offset: %d, sizeof(cl->hdrbuf) = %d\n",
				current_time(), cl->pending_msg_offset, sizeof(cl->hdrbuf));
			goto out;
		}

		if (blob_pad_len(&cl->hdrbuf.data) > UBUS_MAX_MSGLEN) {
			printf("%s|ubus|ubusd|client_cb| recv msg len error, blob_pad_len(&cl->hdrbuf.data) = %d,  UBUS_MAX_MSGLEN = %d\n",
				current_time(), blob_pad_len(&cl->hdrbuf.data), UBUS_MAX_MSGLEN);
			goto disconnect;
		}

		cl->pending_msg = ubus_msg_new(NULL, blob_raw_len(&cl->hdrbuf.data), false);
		if (!cl->pending_msg) {
			printf("%s|ubus|ubusd|client_cb| pending msg error\n",current_time());
			goto disconnect;
		}
	...
	if (ub) {
		...
		if (bytes < len) {
			cl->pending_msg_offset += bytes;
			printf("%s|ubus|ubusd|client_cb| recv msg len error, bytes: %d, len = %d\n",current_time(), bytes, len);
			goto out;
		}
...
{
	printf("%s|ubus|ubusd|server_cb|events: %d\n",current_time(), events);
	bool next;
...
{
	CPE_LOG_DEBUG("===========================ubusd start===========================\n");
	CPE_LOG_DEBUG("build time:%s %s, version:%s\n", __DATE__, __TIME__, UBUSD_VERSION);	
