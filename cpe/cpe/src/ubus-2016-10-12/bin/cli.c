...
{
	fprintf(stderr, "ubus|cli|format_type\n");
...
{
	fprintf(stderr, "ubus|cli|receive_list_result\n");
...
{
	fprintf(stderr, "ubus|cli|receive_call_result_data|type: %d\n", type);
	char *str;
	if (!msg)
		return;
...
{
	fprintf(stderr, "ubus|cli|receive_event|type: %s\n", type);
	char *str;
...
{
	fprintf(stderr, "ubus|cli|ubus_cli_list|argc: %d\n", argc);
...
{
	fprintf(stderr, "ubus|cli|ubus_cli_call|argc: %d\n", argc);
	uint32_t id;
	int ret;
...
{
	fprintf(stderr, "ubus|cli|listen_timeout\n");
...
{
	fprintf(stderr, "ubus|cli|ubus_cli_listen|argc: %d\n", argc);
...
{
	fprintf(stderr, "ubus|cli|ubus_cli_send|argc: %d\n", argc);
	if (argc < 1 || argc > 2)
		return -2;
...
{
	fprintf(stderr, "ubus|cli|wait_check_object|path: %s\n", path);
	int i;
...
static void wait_event_cb(struct ubus_context *ctx, struct ubus_event_handler *ev,
			  const char *type, struct blob_attr *msg)
{
	fprintf(stderr, "ubus|cli|wait_event_cb|type: %s\n", type);
...
{
	fprintf(stderr, "ubus|cli|wait_list_cb\n");
...
{
	fprintf(stderr, "ubus|cli|wait_timeout\n");
	uloop_end();
}

static int ubus_cli_wait_for(struct ubus_context *ctx, int argc, char **argv)
{
	fprintf(stderr, "ubus|cli|ubus_cli_wait_for|argc: %d\n", argc);
...
{
	fprintf(stderr, "ubus|cli|ubus_cli_msg_type\n");
	const char *ret = NULL;
...
{
	fprintf(stderr, "ubus|cli|ubus_cli_get_monitor_data\n");
...
{
	fprintf(stderr, "ubus|cli|ubus_cli_monitor_cb\n");
...
{
	fprintf(stderr, "ubus|cli|ubus_cli_monitor\n");
	int ret;
...
{
	fprintf(stderr, "ubus|cli|add_monitor_type|type: %s\n", type);
	int i;
