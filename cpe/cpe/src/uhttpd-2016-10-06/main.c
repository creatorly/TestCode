...
#define UHTTPD_VERSION "1.0.2"

char uh_buf[4096];

static int run_server(void)
{
	CPE_LOG_DEBUG("===========================uhttpd start===========================");
	CPE_LOG_DEBUG("build time:%s %s, version:%s", __DATE__, __TIME__, UHTTPD_VERSION);
...
int main(int argc, char **argv)
{
	...
	while ((ch = getopt(argc, argv, "A:aC:c:Dd:E:fh:H:I:i:K:k:L:l:m:N:n:p:qRr:Ss:T:t:U:u:Xx:y:")) != -1) {
		fprintf(stderr, "uhttpd|main|getopt|ch:%c\n", ch);
		switch(ch) {
#ifdef HAVE_TLS
		case 'C':
			tls_crt = optarg;
