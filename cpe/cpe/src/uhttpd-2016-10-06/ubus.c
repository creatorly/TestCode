...
static void uh_ubus_send_response(struct client *cl)
{
	...
	printf("uh_ubus_send_response, str = %s \n", str);
...
{
	printf("%s|uhttpd|ubus|uh_ubus_json_error\n", current_time());
...
{
	printf("%s|uhttpd|ubus|uh_ubus_request_data_cb|type: %d\n", current_time(), type);
...
{
	printf("%s|uhttpd|ubus|uh_ubus_request_cb|ret: %d\n", current_time(), ret);
	struct dispatch_ubus *du = container_of(req, struct dispatch_ubus, req);
...
{
	printf("%s|uhttpd|ubus|uh_ubus_timeout_cb\n", current_time());
...
{
	printf("%s|uhttpd|ubus|===== uh_ubus_close_fds =====\n", current_time());
	if (ctx->sock.fd < 0)
		return;
...
{
	printf("%s|uhttpd|ubus|===== uh_ubus_request_free =====\n", current_time());
	struct dispatch_ubus *du = &cl->dispatch.ubus;
...
{
	printf("%s|uhttpd|ubus|uh_ubus_single_error\n", current_time());
...
{
	printf("%s|uhttpd|ubus|uh_ubus_send_request|sid: %s\n", current_time(), sid);
...
{
	printf("%s|uhttpd|ubus|uh_ubus_list_cb\n", current_time());
...
{
	printf("%s|uhttpd|ubus|uh_ubus_send_list\n", current_time());
...
{
	printf("%s|uhttpd|ubus|uh_ubus_allowed_cb|type: %d\n", current_time(), type);
...
{
	printf("%s|uhttpd|ubus|uh_ubus_allowed|sid: %s, obj: %s, fun: %s\n", current_time(), sid, obj, fun);
	uint32_t id;
	bool allow = false;
	static struct blob_buf req;
	...
	// return  allow;
	printf("%s|uhttpd|ubus|uh_ubus_allowed|return true\n", current_time());
	return true;
}

static void uh_ubus_handle_request_object(struct client *cl, struct json_object *obj)
{
	printf("%s|uhttpd|ubus|uh_ubus_handle_request_object\n", current_time());
	struct dispatch_ubus *du = &cl->dispatch.ubus;
...
{
	printf("%s|uhttpd|ubus|uh_ubus_data_send|data: %s, len: %d\n", current_time(), data, len);
	struct dispatch_ubus *du = &cl->dispatch.ubus;
...
{
	printf("%s|uhttpd|ubus|uh_ubus_handle_request|url: %s\n", current_time(), url);
	struct dispatch *d = &cl->dispatch;
...
uh_ubus_init(void)
{
	printf("%s|uhttpd|ubus|===== uh_ubus_init =====\n", current_time());
...
{
	fprintf(stderr, "uhttpd|ubus|uh_ubus_plugin_init\n");
	ops = o;
	_conf = c;
	return uh_ubus_init();
}
...
{
	fprintf(stderr, "uhttpd|ubus|uh_ubus_post_init\n");
	ubus_add_uloop(ctx);
}
