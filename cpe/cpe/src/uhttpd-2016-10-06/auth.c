...
bool uh_auth_check(struct client *cl, struct path_info *pi)
{
	if (NULL != pi)
	{
		printf("%s|uhttpd|auth|uh_auth_check|root: %s, phys: %s\n", current_time(), pi->root, pi->phys);
	} else	{
		printf("%s|uhttpd|auth|uh_auth_check|pi is empty\n", current_time());
	}
...
//		return true;
	...
	printf("%s|uhttpd|auth|uh_auth_check|return true\n", current_time());
	return true;
/*
