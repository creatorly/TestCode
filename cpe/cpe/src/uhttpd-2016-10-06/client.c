...
{
	printf("%s|uhttpd|client|uh_http_header|code: %d, summary: %s\n", current_time(), code , summary);
...
{
	printf("%s|uhttpd|client|===== uh_connection_close =====\n", current_time());
...
{
	printf("%s|uhttpd|client|===== uh_dispatch_done =====\n",  current_time());
	if (cl->dispatch.free)
		cl->dispatch.free(cl);
...
{
	printf("%s|uhttpd|client|===== uh_poll_connection =====\n", current_time());
...
{
	printf("%s|uhttpd|client|===== uh_request_done =====\n", current_time());
	uh_chunk_eof(cl);
	uh_dispatch_done(cl);
...
{
	printf("%s|uhttpd|client|uh_client_error|code:%d, summary:%s, fmt:%s\n", current_time(), code, summary, fmt);
	va_list arg;
...
{
	printf("%s|uhttpd|client|uh_header_error|code:%d, summary:%s\n", current_time(), code, summary);
...
{
	printf("%s|uhttpd|client|client_parse_request|data:%s\n", current_time(), data);
...
{
	printf("%s|uhttpd|client|client_init_cb|buf:%s\n", current_time(), buf);
...
{
	printf("%s|uhttpd|client|rfc1918_filter_check|return true\n", current_time());
	return true;
	// cdj remove code
/*
...
{
	printf("%s|uhttpd|client|tls_redirect_check|return true\n", current_time());
	return true;
	// cdj remove code
/*
	int rem, port;
...
{
	printf("%s|uhttpd|client|===== client_header_complete =====\n", current_time());
	struct http_request *r = &cl->request;
...
{
	printf("%s|uhttpd|client|client_data_cb|buf:%s\n", current_time(), buf);
...
{
	printf("%s|uhttpd|client|client_header_cb|buf:%s\n", current_time(), buf);
	char *newline;
	int line_len;
...
{
	printf("%s|uhttpd|client|uh_client_read_cb\n", current_time());
...
{
	printf("%s|uhttpd|client|===== client_close =====\n", current_time());
	if (cl->refcount) {
...
{
	printf("%s|uhttpd|client|uh_accept_client|fd: %d\n", current_time(), fd);
...
{
	printf("%s|uhttpd|client|===== uh_close_fds =====\n", current_time());
	struct client *cl;
