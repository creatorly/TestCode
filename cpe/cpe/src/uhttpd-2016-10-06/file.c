...
{
	printf("%s|uhttpd|file|uh_index_add|filename:%s\n", current_time(), filename);
...
next:
	...
	*path_res = '\0';
	...
	fprintf(stderr, "uhttpd|file|canonpath|path_resolved:%s\n", path_resolved );
...
{
	printf("%s|uhttpd|file|uh_file_response_ok_hdrs\n", current_time());
	char buf[128];
...
{
	printf("%s|uhttpd|file|uh_file_response_200\n", current_time());
...
{
	printf("%s|uhttpd|file|uh_file_response_304\n", current_time());
...
{
	printf("%s|uhttpd|file|uh_file_response_412\n", current_time());
			    ...
			    struct path_info *pi, struct blob_attr **tb)
{
	...
	/* fix read log file 20190109 */
	//if (!(pi->stat.st_mode & S_IROTH)) {
	//	CPE_LOG_ERROR("file stat error, pi->stat.st_mode =%d, url = %s\n", pi->stat.st_mode, url);
	//	goto error;
	//}
	...
	if (pi->stat.st_mode & S_IFREG) {
		...
		if (fd < 0) {
			CPE_LOG_ERROR("open file error, pi->stat.st_mode =%d", pi->stat.st_mode);
			goto error;
		}
	...
	if ((pi->stat.st_mode & S_IFDIR)) {
		if (conf.no_dirlists) {
			CPE_LOG_ERROR("open file no dirlists");
			goto error;
		}
...
{
	if (NULL != pi)
		printf("%s|uhttpd|file|uh_invoke_handler|root%s, phys:%s, url:%s\n", current_time(), pi->root, pi->phys, url);
	else
		printf("%s|uhttpd|file|uh_invoke_handler|url:%s, and pi is empty\n", current_time(), url);
...
static char *uh_handle_alias(char *old_url)
{
	...
	if (!list_empty(&conf.cgi_alias)) list_for_each_entry(alias, &conf.cgi_alias, list) {
		...
		fprintf(stderr, "uhttpd|file|uh_handle_alias|old_len: %d, new_len: %d\n", old_len, new_len);
		...
		fprintf(stderr, "uhttpd|file|uh_handle_alias|init new_url\n");
		*new_url = '\0';
		...
		fprintf(stderr, "uhttpd|file|uh_handle_alias|return new_url\n");
		return new_url;
	}

	fprintf(stderr, "uhttpd|file|uh_handle_alias|return old_url\n");
	return old_url;
}
...
//	url = uh_handle_alias(url);
	...
	printf("%s|uhttpd|file|uh_handle_request|url: %s\n", current_time(), url);
	uh_handler_run(cl, &url, false);
	if (!url)
	{
		printf("%s|uhttpd|file|uh_handle_request|url empty\n", current_time());
		return;
	}
	...
	if (d)
	{
		printf("%s|uhttpd|file|uh_handle_request|dispatch find\n", current_time());
		return uh_invoke_handler(cl, d, url, NULL);
	}

	if (__handle_file_request(cl, url))
	{
		printf("%s|uhttpd|file|uh_handle_request|__handle_file_request\n", current_time());
		return;
	}
