...
MIPS_FLAGS="-pipe -mno-branch-likely -mips32r2 -mtune=24kec -mdsp -fno-caller-saves -fhonour-copts -msoft-float --sysroot=$TOOLSCHAIN $EXTRA_DEFINE"
INCLUDES="-I$TOOLSCHAIN/include -I$TOOLSCHAIN/usr/include -I$TOP_SRC/../config"

CC=$TOOLSCHAIN/bin/mipsel-openwrt-linux-gcc \
CXX=$TOOLSCHAIN/bin/mipsel-openwrt-linux-g++ \
AR=$TOOLSCHAIN/bin/mipsel-openwrt-linux-ar \
LD=$TOOLSCHAIN/bin/mipsel-openwrt-linux-ld \
LIBC=$TOOLSCHAIN/lib \
...
./configure --enable-shared=yes --prefix=$VAD_PREFIX \
 ...
 --with-arch=mips --build=i686-pc-linux --host=mips-linux-uclibc --cache-file=mips-linux.cache \
 --with-board-type=$ACOM_BOARD_TYPE --with-hw-type=$ACOM_HW_TYPE --with-oem-name=$ACOM_OEM_NAME \
 CFLAGS="$MIPS_FLAGS $INCLUDES" CXXFLAGS="$MIPS_FLAGS $INCLUDES"
