...
typedef enum
{
    ...
    UTL_CONN_TYPE_TLSC,     /* TLS Client connection */
    UTL_CONN_TYPE_TLSS,     /* TLS Server connection */
...
typedef enum
{
    ...
    UTL_CONN_PAR_SSLCTX,        /* SSL contex */
    UTL_CONN_PAR_HOST,          /* the hostname of remote, needed by https client */
...
typedef void (*FP_UTL_CONN_ONSENT)(utl_conn_t* pcon, void* usrDat);

const char* utl_conn_type2str(int type);
...
/* when each data is sent completedly, the function fpSent will be invoked */
void utl_conn_set_hook_onsent(utl_conn_t* conn, FP_UTL_CONN_ONSENT fpSent, void* data);
